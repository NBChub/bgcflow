...
# remove old arts folder
if [ -d "resources/arts" ]; then
    rm -rf resources/arts 2>> "$ARTS_LOG_DIR/arts_setup.log"
fi

if [ -f "resources/arts.zip" ]; then
    rm resources/arts.zip 2>> "$ARTS_LOG_DIR/arts_setup.log"
fi

echo "Downloading ARTS..." &>> "$ARTS_LOG_DIR/arts_setup.log"

wget -O resources/arts.zip "$ARTS_REPO/archive/refs/tags/$ARTS_RELEASE_VERSION.zip" 2>> "$ARTS_LOG_DIR/arts_setup.log"

if [ -d "arts_v3-$ARTS_RELEASE_VERSION" ]; then
...
wget -P resources/ "$REF_DB" 2>> "$ARTS_LOG_DIR/arts_setup.log"
