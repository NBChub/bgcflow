...
def copy_automlst_out(automlst_interim_folder, automlst_processed_folder):
    ...
    files_to_copy = ['raxmlpart.txt', 'raxmlpart.txt.bionj', 'raxmlpart.txt.contree', 'raxmlpart.txt.iqtree', 'raxmlpart.txt.log',
                        'raxmlpart.txt.mldist', 'raxmlpart.txt.treefile', ]
...
def get_genome_tree_table(automlst_processed_folder, prokka_interim_folder, gtdb_interim_folder):
    ...
    columns_list = ['genus_original', 'species_original', 'strain', 'domain', 'phylum', 'class', 'order', 'family', 'genus', 'species']
    ...
    for genome_id in df_genomes_tree.index:
        ...
        org_info_path = os.path.join(prokka_interim_folder, genome_id, 'organism_info.txt')
        ...
        with open(gtdb_json_path, "r") as f:
            ...
            df_genomes_tree.loc[genome_id, 'domain'] = gtdb_dict['gtdb_taxonomy']['domain']
            df_genomes_tree.loc[genome_id, 'phylum'] = gtdb_dict['gtdb_taxonomy']['phylum']
            df_genomes_tree.loc[genome_id, 'class'] = gtdb_dict['gtdb_taxonomy']['class']
            df_genomes_tree.loc[genome_id, 'order'] = gtdb_dict['gtdb_taxonomy']['order']
            df_genomes_tree.loc[genome_id, 'family'] = gtdb_dict['gtdb_taxonomy']['family']
            df_genomes_tree.loc[genome_id, 'genus'] = gtdb_dict['gtdb_taxonomy']['genus']
            df_genomes_tree.loc[genome_id, 'organism'] = gtdb_dict['gtdb_taxonomy']['species']
            try:
                df_genomes_tree.loc[genome_id, 'species'] = gtdb_dict['gtdb_taxonomy']['species'].split(' ')[1]
...
if __name__ == "__main__":
    ...
    df_genomes_tree = get_genome_tree_table(automlst_processed_folder, prokka_interim_folder, gtdb_interim_folder)
