...
def get_checkm_data(checkm_input_stats, checkm_json_folder, checkm_output_stats):
    ...
    checkm_columns = ['Completeness', 'Contamination', 'GC', 'GC std', 'Genome size', '# ambiguous bases', '# scaffolds', 
                '# contigs', 'Longest scaffold', 'Longest contig', 'N50 (scaffolds)', 'N50 (contigs)', 'Mean scaffold length', 
                'Mean contig length', 'Coding density', 'Translation table', '# predicted genes']
    
    df_checkm_input = pd.read_csv(checkm_input_stats, sep='\t',header=None, index_col=0)
