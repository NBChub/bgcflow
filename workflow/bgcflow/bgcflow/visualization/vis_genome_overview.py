...
def plot_bgc_dist(
    df_genomes, col_select="bgcs_count", to_path="../figures/bgc_dist.pdf"
):
    """
    ...
    plot_hist(
        df_genomes, col_select, bin_size, title_str, xlabel_str, ylabel_str, to_path
    )
...
def scatter_bgcs_len(df_genomes, to_path="../figures/scatter_bgcs_len.pdf"):
    ...
    df_counts = (
        scat_data.groupby(["genome_len", "bgcs_count", "genus"])
        .size()
    ...
    ax_right.hist(
        scat_data["bgcs_count"],
        int(max(scat_data.bgcs_count) + 1),
        histtype="stepfilled",
    ...
    ax_main.set(
        title="No. of clusters vs Genome Length",
        xlabel="Genome Length (Mb)",
        ylabel="No of clusters per genome",
    )
    ...
    for item in (
        [ax_right.xaxis.label, ax_right.yaxis.label]
